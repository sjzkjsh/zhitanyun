...
public class ThresholdRangeService extends ServiceImpl<ThresholdRangeMapper, ThresholdRange> {
    @Autowired
    protected ThresholdRangeMapper configMapper;

    /**
     * 获取阈值范围（带缓存）
     * 优先级：设备级 > 建筑级 > 全局默认
     */
    @Cacheable(value = "thresholdRange",
            key = "#buildingId + ':' + #deviceId + ':' + #metricName + ':' + #time.toLocalDate()",
            unless = "#result == null")
    ...
    public Map<String, Range> getRangesBatch(Integer buildingId, Integer deviceId,
                                             List<String> metrics, LocalDateTime referenceTime) {
        if (metrics == null || metrics.isEmpty()) {
            return Collections.emptyMap();
        }

        // 1. 一次查询获取所有相关配置（传入过滤参数）
        List<ThresholdRange> configs = configMapper.selectBatchByMetrics(metrics, referenceTime, buildingId, deviceId);

        if (configs == null || configs.isEmpty()) {
            log.debug("未查询到任何阈值配置, buildingId={}, deviceId={}, metrics={}", buildingId, deviceId, metrics);
            return Collections.emptyMap();
        }

        // 2. 转换为 Map<指标名, Range>
        //    如果有同一指标多条配置（如按优先级），取第一条
        return configs.stream()
                .collect(Collectors.toMap(
                        ThresholdRange::getMetricName,
                        config -> new Range(
                                config.getMinValue(),
                                config.getMaxValue(),
                                config.getUnit(),
