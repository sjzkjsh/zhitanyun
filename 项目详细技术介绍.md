...
| 技术 | 版本 | 用途 |
|------|------|------|
| MySQL | 8.x | 主从复制（Master 3306 + Slave 3307），读写分离 |
| ProxySQL | 2.x | MySQL 中间代理，自动路由写→主库、读→从库 |
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                    ProxySQL（读写分离代理）                │   │
│  │         应用统一连接 ProxySQL 端口，自动路由读写           │   │
│  └────────────┬────────────────────────┬────────────────────┘   │
│               │ 写操作                  │ 读操作                 │
│               ▼                        ▼                       │
│  ┌─────────────────────┐  ┌─────────────────────┐              │
│  │   MySQL Master      │  │   MySQL Slave       │              │
│  │   主库（端口 3306）   │  │   从库（端口 3307）  │              │
│  │   处理 INSERT/UPDATE │  │   处理 SELECT       │              │
│  │   DELETE 等写操作    │  │   主从复制实时同步   │              │
│  └──────────┬──────────┘  └──────────┬──────────┘              │
│             └───── 主从复制 ──────────┘                         │
│                                                                │
│  ┌──────────────┐  ┌──────────────────────────────────────┐    │
...
### 5.4 MySQL 主从复制与 ProxySQL 读写分离

**架构说明**：系统采用 MySQL 主从复制 + ProxySQL 中间代理的方式实现读写分离，应用层无需感知读写分离逻辑，由 ProxySQL 根据 SQL 语句自动路由。
...
| 配置项 | 值 | 说明 |
|--------|------|------|
| mysql-monitor_username | monitor | 监控账号 |
| mysql-monitor_password | *** | 监控密码 |
| hostgroup 0 | Master (3306) | 写组 |
| hostgroup 1 | Slave (3307) | 读组 |
| query_rules | SELECT → hg1 | 读请求路由到从库 |
| max_connections | 1000 | 最大连接数 |

**MySQL 主从复制配置**：
| 角色 | 端口 | 说明 |
|------|------|------|
| Master | 3306 | 接受所有写操作，生成 binlog |
| Slave | 3307 | 通过 IO 线程拉取 binlog，SQL 线程回放 |
| 复制模式 | 异步 | Master 不等待 Slave 确认，延迟 < 1s |

**对应用的价值**：
- 读请求分散到从库，减轻主库压力，**读吞吐量提升 ~2 倍**
- 写请求集中在主库，保证数据一致性
- 主库故障时可通过 ProxySQL 手动切换到从库
- 应用代码无需修改，统一连 ProxySQL 端口即可
